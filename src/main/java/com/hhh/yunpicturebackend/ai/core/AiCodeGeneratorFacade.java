...
public class AiCodeGeneratorFacade {
    ...
    public List<String> generateImageUnderstand(AiChatWithImageService service,List<String> imageUrls) {
        // 结果列表
        ...
        for (String imageUrl : imageUrls) {
            ...
            try {
                ...
                // 调用AI（单张图识别）
                String understandResult = service.chatWithImage("识别图片",imageContent);

                // 把结果存入列表
                resultList.add(imageUrl+":"+understandResult);

            } catch (Exception e) {
                // 失败兜底
                resultList.add(imageUrl + ": 图片识别失败");
                e.printStackTrace();
            }
        }
     ...
     */
    public List<String> getOrCreateImageUnderstand(long appId, List<String> imageUrls) {
        if (imageUrls == null || imageUrls.isEmpty()) {
            return List.of();
        }
        String cacheKey = String.valueOf(appId);
        // 🔥 核心：不存在才执行，存在直接返回缓存
        return aiCodeGeneratorServiceFactory.imageResultCache.get(cacheKey, key -> {
            // 只有第一次会执行这里
            AiChatWithImageService service = aiCodeGeneratorServiceFactory.createAiPictureUnderstandService();
            return generateImageUnderstand(service, imageUrls);
        });
    }
    /**
     * 通用流式代码处理方法
    ...
    public Flux<String> generateAndSaveCodeStream(String userMessage, List<String> imageUrls , CodeGenTypeEnum codeGenTypeEnum, Long appId) {
            ...
            List<String> generateImageUnderstand=getOrCreateImageUnderstand(appId,imageUrls);

        if (codeGenTypeEnum == null) {
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "生成类型为空");
        }
        // 【关键修复】强制判空：如果 imageUrls 为 null，就替换为空列表
        List<String> safeImageUrls = (imageUrls != null) ? imageUrls : new ArrayList<>();
        // 根据 appId 获取对应的 AI 服务实例
        AiCodeGeneratorService aiCodeGeneratorService = aiCodeGeneratorServiceFactory.getAiCodeGeneratorService(appId, codeGenTypeEnum);
        return switch (codeGenTypeEnum) {
            case HTML -> {
                Flux<String> codeStream = aiCodeGeneratorService.generateHtmlCodeStream(userMessage,safeImageUrls,generateImageUnderstand);
                yield processCodeStream(codeStream, CodeGenTypeEnum.HTML,appId);
            }
            case MULTI_FILE -> {
                Flux<String> codeStream = aiCodeGeneratorService.generateMultiFileCodeStream(userMessage,safeImageUrls,generateImageUnderstand);
                yield processCodeStream(codeStream, CodeGenTypeEnum.MULTI_FILE,appId);
            }
            case VUE_PROJECT -> {
                TokenStream tokenStream = aiCodeGeneratorService.generateVueProjectCodeStream(appId, userMessage,safeImageUrls,generateImageUnderstand);
