...
import com.hhh.yunpicturebackend.ai.guardrail.RetryOutputGuardrail;
import com.hhh.yunpicturebackend.ai.tools.*;
...
import static com.hhh.yunpicturebackend.ai.enums.CodeGenTypeEnum.*;
...
*/
    ...
    @Resource
    private StreamingChatModel ReasoningGeminiStreamingChatModel;*/
    /**
     * AI 服务实例缓存
     */
     ...
     * AI 服务实例缓存
     */
    private final Cache<String, AiChatWithImageService> Cache = Caffeine.newBuilder()
            .maximumSize(1000)
            .expireAfterWrite(Duration.ofMinutes(30))
            .expireAfterAccess(Duration.ofMinutes(10))
            .removalListener((key, value, cause) -> {
            ...
            .build();
    // ===================== 新增：缓存 图片理解结果 =====================
    public final Cache<String, List<String>> imageResultCache = Caffeine.newBuilder()
            .maximumSize(1000)
            .expireAfterWrite(Duration.ofMinutes(30))
            .build();
     ...
     * 根据 appId 获取识图服务
     */
    public AiChatWithImageService createAiPictureUnderstandService() {
        ChatModel chatModel = SpringContextUtil.getBean("ImageUnderstandPrototype", ChatModel.class);
