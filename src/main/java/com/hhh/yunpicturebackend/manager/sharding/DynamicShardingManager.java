package com.hhh.yunpicturebackend.manager.sharding;
...
public class DynamicShardingManager {
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream()
                .map(tableName -> "hhh_picture." + tableName) // 确保前缀合法
     ...
     * 创建空间图片动态分表
     */
    public void creatSpacePictureTable(Space  space) {
        //仅对旗舰版团队空间生效创建分表
        if (space.getSpaceType() == SpaceTypeEnum.TEAM.getValue() && space.getSpaceLevel() == SpaceLevelEnum.FLAGSHIP.getValue()) {
            ...
            // 创建动态表
            String createTabsql = "CREATE TABLE " + tableName + " LIKE " + LOGIC_TABLE_NAME;
            try {
                SqlRunner.db().update(createTabsql);
                //更新分表
                updateShardingTableNodes();
            }catch (Exception e){
                log.error("创建空间图片动态分表失败！空间id = {}", space.getId());
            }
        }
    }
    /**
